...
const isDemoSession = (user: any) =>
  user?.id === 'demo-admin' ||
  user?.id === 'demo-nurse' ||
  user?.email === 'admin@remeinia.org' ||
  user?.email === 'enfermera.demo@remeinia.org';

const toMotorInput = (body: any): ValoracionFormData => ({
...
export async function POST(req: NextRequest) {
  ...
  if (isDemoSession(user)) {
    ...
    return NextResponse.json({
      ...
      plan: resultado.plan,
      advertenciaGeneral: resultado.advertenciaGeneral,
    });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: 'No fue posible procesar la valoración' }, { status: 500 });
  }
}
